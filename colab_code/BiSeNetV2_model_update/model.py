...
def context_embedding(x_in, c):
    ...
    x = layers.AveragePooling2D((8,16),padding='valid')(x_in)
    x = layers.BatchNormalization()(x)
...
def bilateral_guided_aggregation(detail, semantic, c):
    ...
    semantic_b = layers.Conv2DTranspose(128,kernel_size=(4, 4),strides=(4, 4),padding='same')(semantic_b)
    ...
    semantic = layers.Conv2DTranspose(128,kernel_size=(4, 4),strides=(4, 4),padding='same')(semantic)
...
def seg_head(x_in, c_t, s, n):
    ...
    x = layers.Conv2DTranspose(2,kernel_size=(12, 12),strides=(s, s),padding='same')(x)
    return x
